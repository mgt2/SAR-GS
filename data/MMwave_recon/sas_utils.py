...
def hilbert_torch_batch(x):
    N = x.shape[-1]

    # Record if we need to squeeze later
    ...
    # Take forward Fourier transform
    Xf = torch.fft.fft(x, dim=-1)
    h = torch.zeros_like(x, dtype=torch.float32)
    ...
    # Apply the Hilbert transform filter
    Xf_filtered = Xf * h.to(Xf.device)
...
def crop_wfm(tx_coords, rx_coords, corners, wfm_length, fs, K, speed_of_sound, pad=.05):
    ...
    min_dist = (d1 + d2).ravel().min() - pad - (min(wfm_length, 100) * speed_of_sound/(2*K))
    max_dist = (d1 + d2).ravel().max() + pad + (min(wfm_length, 100) * speed_of_sound/(2*K))

    assert max_dist > min_dist, "Sanity check failed"

    min_sample = math.floor(min_dist / (speed_of_sound*fs) * K * wfm_length)
    max_sample = math.floor(max_dist / (speed_of_sound*fs) * K * wfm_length)

    # Update the min dist based off the rounded down sample
    min_dist = min_sample*fs * speed_of_sound/(K*wfm_length)
    max_dist = max_sample*fs * speed_of_sound/(K*wfm_length)

    # 340 is a conservative sound speed to use.
    # t_dur = (max_dist - min_dist) / speed_of_sound
    max_sample = 150

    num_samples = max_sample - min_sample

    # Update the max dist based off the rounded up sample
    # max_dist = ((min_sample + num_samples) / fs) * speed_of_sound

    wfm_crop_settings = WfmCropSettings()
    wfm_crop_settings[c.MIN_SAMPLE] = 0 #min_sample
    wfm_crop_settings[c.MIN_DIST] = 0 #min_dist
    wfm_crop_settings[c.MAX_DIST] = 10.661 #max_dist
    wfm_crop_settings[c.NUM_SAMPLES] = 256 #num_samples
